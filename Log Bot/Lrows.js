...
client.t0kenlrows = "ODU1MDY0ODM4NzIzNzMxNDY2.YMtDLw.wFxrnRJRtm_qr0cqrEurMIbUVVg";//Botunuzun Tokeni - Lrows
client.log = "870771075821240340";//Logların Gideceği Kanal - Lrows


const send = async function send(type, content, imageURL) {
const embed = new MessageEmbed().setColor("RANDOM").setTimestamp().setTitle(type).setDescription(content).setFooter("Lrows")
...
client.login(client.t0kenlrows).then(Wenzy => console.log(`${client.user.tag} ismi ile giriş yapıldı!`)).catch(lrowshata => console.error("Bota giriş yapılamadı."));

client.on("ready", () => {
send("Bot Aktif Oldu!", `Bota başarıyla giriş yapıldı!`);
console.log("Bota başarıyla giriş yapıldı!");
client.user.setPresence({ activity: { type: "PLAYING", name: `Lrows Log Bot`}, status: 'dnd' });
})
